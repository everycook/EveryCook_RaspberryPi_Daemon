...
uint32_t ConfigurationReg[] = {0x390, 0x391, 0x392, 0x393, 0x914, 0x295};//For AD7796 conf-reg
...
*/
void AD7796Init(void){
	if (debug_enabled2){printf("AD7796Init\n");}
	if (use_spi_dev){
		int r;
		...
		ad7796_communicate(&adc, AD7796_MODE, AD7796_DIRECTION_WRITE, 2, &value[0]);
	} else {
		SPISelectChip(0);
		SPIReset();
		delay(30);
...
uint32_t readADC(uint8_t i){
	uint32_t data;
	if (use_spi_dev){
		ad7796_select_channel2(&adc, i, ConfigurationReg[i]);
	} else {
	...
	if (use_spi_dev){
		while (!readyToRead){//wait until the transmition is ready
			if (ad7796_check_if_ready(&adc)){//if ready
				readyToRead = true;
			} else {//not ready wait
				delay(5);
			}
